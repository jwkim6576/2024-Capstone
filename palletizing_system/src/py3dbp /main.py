...
from .auxiliary_methods import intersect, set2Decimal,RPY
import numpy as np
...
class Item:
    ...
    def getCenterPos(self):
        ...
        y = 10
        if self.rotation_type == RotationType.RT_WHD:
            centerpos = (self.position[0] + self.width/2 + x, self.position[1] + self.height/2 + y, self.position[2] + self.depth)
        elif self.rotation_type == RotationType.RT_HWD:
            centerpos = (self.position[0] + self.height/2 + x, self.position[1] + self.width/2 + y, self.position[2] + self.depth)
        elif self.rotation_type == RotationType.RT_HDW:
            centerpos = (self.position[0] + self.height/2 + x, self.position[1] + self.depth/2 + y, self.position[2] + self.width)
        elif self.rotation_type == RotationType.RT_DHW:
            centerpos = (self.position[0] + self.depth/2 + x, self.position[1] + self.height/2 + y, self.position[2] + self.width)
        elif self.rotation_type == RotationType.RT_DWH:
            centerpos = (self.position[0] + self.depth/2 + x, self.position[1] + self.width/2 + y, self.position[2] + self.height)
        elif self.rotation_type == RotationType.RT_WDH:
            centerpos = (self.position[0] + self.width/2 + x, self.position[1] + self.depth/2 + y, self.position[2] + self.height)
        else:
            centerpos = ()
    ...
    def getOrientation(self):
        ...
        if self.rotation_type == RotationType.RT_WHD:
            ...
            self.orientation1 = "Can't get orientation"

        rpy1 = RPY(angle)
        rpy2 = RPY(angle1)

        self.orientation1 = rpy1.calculate_quaternion()
        self.orientation2 = rpy2.calculate_quaternion()
        self.orientation3 = rpy1.calculate_rotation_matrix()
        self.orientation4 = rpy2.calculate_rotation_matrix()
        return self.orientation3, self.orientation4

    def getRoundedOrientation(self, round_to=2):
        """
        Return the orientation quaternion rounded to the specified number of decimal places.
        :param round_to: Number of decimal places to round to.
        :return: Rounded quaternion [w, x, y, z]
        """
        return [round(value1, round_to) for value1 in self.orientation1], [round(value2, round_to) for value2 in self.orientation2]
...
class Painter:
    ...
    def plotBoxAndItems(self, ax, title="", alpha=0.2, write_num=False, fontsize=10):
        """ side effective. Plot the Bin and the items it contains. """

        # Clear the axes to prevent overlapping drawings
        ax.cla()
